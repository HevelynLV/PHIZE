...
O motor opera sobre dois grupos de sinais: o grupo Texto/Print (os oito sinais listados acima) e o grupo Link, derivado da verificação de URL (RF03/UC03). Quando a verificação de uma fonte externa fica incompleta e a faixa resultante dos sinais disponíveis seria a verde, a faixa exibida é elevada para amarelo, preservando a pontuação real e informando ao usuário qual verificação não pôde ser concluída. **`docs/score-calibracao.md` é a fonte única dos valores numéricos** (pesos, pontos de corte das faixas e cenários de validação) — nenhum outro documento ou trecho de código deve duplicar esses números.
